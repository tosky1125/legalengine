...
const mapIds = (data, keyName) => (Array.isArray(data) ? data : []).map((value) => appendId(value, keyName));
const useFieldArray = ({ control, name, keyName = 'id', }) => {
    ...
    const prepend$1 = (value, shouldFocus = true) => {
        ...
        const updatedFieldArrayValues = prepend(getCurrentFieldsValues(), Array.isArray(value)
            ? appendValueWithKey(value)
            : [appendId(value, keyName)]);
    ...
    const insert$1 = (index, value, shouldFocus = true) => {
        const emptyArray = fillEmptyArray(value);
        const fieldValues = getCurrentFieldsValues();
        const updatedFieldArrayValues = insert(fieldValues, index, Array.isArray(value)
            ? appendValueWithKey(value)
            : [appendId(value, keyName)]);
        setFieldAndValidState(updatedFieldArrayValues);
        resetFields();
